...
namespace VOC.Application.Implementation
{
    public class RoleService : IRoleService
    {
        ...
        private IRespository<PERMISSION, int> _permissionRepository;
        private IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        public RoleService(RoleManager<APP_ROLE> roleManager, IUnitOfWork unitOfWork,
         IRespository<FUNCTION, string> functionRepository, IRespository<PERMISSION, int> permissionRepository, IMapper mapper)
        {
        ...
        public Task<bool> CheckPermission(string functionId, string action, string[] roles)
        {
            ...
            var query = from f in functions
                        ...
                        || (p.CanImport && action == "Import")
                        || (p.CanExport && action == "Export"))
                        select p;
        ...
        public List<AppRoleViewModel> GetAllAsync()
        {
            return _mapper.Map<List<AppRoleViewModel>>(_roleManager.Roles);
        }
        ...
        public PagedResult<AppRoleViewModel> GetAllPagingAsync(string keyword, int page, int pageSize)
        {
            ...
            var data = _mapper.Map<List<AppRoleViewModel>>(query);
        ...
        {
            var role = await _roleManager.FindByIdAsync(id.ToString());
            return _mapper.Map<AppRoleViewModel>(role);
        }
