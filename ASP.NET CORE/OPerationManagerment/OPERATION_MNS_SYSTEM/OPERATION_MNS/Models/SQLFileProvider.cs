...
using OPERATION_MNS.Models.Base;
using OPERATION_MNS.Data.EF.Extensions;

namespace OPERATION_MNS.Models
{
    public class SQLFileProvider : SQLFileProviderBase
    {
        ...
        public SqlConnection setSQLDBConnection()
        {
            ...
            return new SqlConnection(configuration.GetConnectionString("FileManagerConnection"));//(@"" + connectionString);
        }
                    ...
                    try
                    {
                        ...
                        while (reader.Read())
                        {
                            cwd = new FileManagerDirectoryContent
                            {
                                ...
                                DateEx = reader["DateEx"].NullString(),
                                StorageLocation = reader["StorageLocation"].NullString(),
                                USize = ByteConversion((long)reader["Size"])
                            };
        ...
        public long GetFolderSize(string[] idValue)
        {
            ...
            foreach (var id in idValue)
            {
                ...
                string removeQuery = "with cte as (select ItemID, Name, ParentID from " + this.tableName + " where ParentID =" + id + " union all select p.ItemID, p.Name, p.ParentID from FILE_MANAGER p inner join cte on p.ParentID = cte.ItemID) select ItemID from cte;";
        ...
        // Returns the image
        public FileStreamResult GetImage(string path, string parentId, string id, bool allowCompress, Syncfusion.EJ2.FileManager.Base.ImageSize size, params FileManagerDirectoryContent[] data)
        {
            try
            {
        ...
        public FileManagerResponse Delete(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                sqlConnection.Open();
                string removeQuery = "with cte as (select ItemID, Name, ParentID from " + this.tableName + " where ParentID =" + data[0].Id + " union all select p.ItemID, p.Name, p.ParentID from FILE_MANAGER p inner join cte on p.ParentID = cte.ItemID) select ItemID from cte;";
                    ...
                    if (uploadFiles != null)
                    {
                        ...
                        if (action == "save")
                        {
                            if (!System.IO.File.Exists(absoluteFilePath))
                            {
                                ...
                                using (FileStream fsSource = new FileStream(absoluteFilePath, FileMode.Open, FileAccess.Read))
                                {
                                    ...
                                    UploadQuery(fileName, contentType, data[0].StorageLocation, data[0].DateEx, bytes, data[0].Id);
                                }
                            }
                            else
                            {
                        ...
                        else if (action == "keepboth")
                        {
                            ...
                            using (FileStream fsSource = new FileStream(newAbsoluteFilePath, FileMode.Open, FileAccess.Read))
                            {
                                ...
                                byte[] bytes = binaryReader.ReadBytes((int)numBytes);
                                UploadQuery(newFileName, contentType, data[0].StorageLocation, data[0].DateEx, bytes, data[0].Id);
                            }
                        }
                    }
                }
        ...
        public void UploadQuery(string fileName, string contentType,string storageLocation,string dateEx, byte[] bytes, string parentId)
        {
            sqlConnection = setSQLDBConnection();
            sqlConnection.Open();
            SqlCommand command = new SqlCommand("INSERT INTO " + tableName + " (Name, ParentID, Size, IsFile, MimeType, Content, DateModified, DateCreated, HasChild, IsRoot, Type,StorageLocation,DateEx) VALUES ( @Name, @ParentID, @Size, @IsFile, @MimeType, @Content, @DateModified, @DateCreated, @HasChild, @IsRoot, @Type, @StorageLocation, @DateEx)", sqlConnection);
            ...
            command.Parameters.Add(new SqlParameter("@StorageLocation", storageLocation.NullString()));
            command.Parameters.Add(new SqlParameter("@DateEx", dateEx.NullString()));
            command.ExecuteNonQuery();
        }
            ...
            try
            {
                ...
                if (availableFiles.Count > 0)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (searchData.Name != "FILE_MANAGER") foundFiles.Add(searchData);
                    }
                    reader.Close();
        ...
        public FileManagerResponse Copy(string path, string targetPath, string[] names, string[] replacedItemNames, FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                sqlConnection.Open();
                string checkingQuery = "with cte as (select ItemID, Name, ParentID from " + this.tableName + " where ParentID =" + data[0].Id + " union all select p.ItemID, p.Name, p.ParentID from FILE_MANAGER p inner join cte on p.ParentID = cte.ItemID) select ItemID from cte;";
        ...
        public FileManagerResponse Edit(string id, string location, string dateEx)
        {
            FileManagerResponse renameResponse = new FileManagerResponse();
            try
            {
                ...
                {
                    sqlConnection.Open();
                    SqlCommand updatecommand = new SqlCommand(("update " + this.tableName + " set StorageLocation='" + location + "',DateEx='" +dateEx+ "' , DateModified='" + DateTime.Now.ToString() + "' where ItemID ='" + id + "'"), sqlConnection);
                    updatecommand.ExecuteNonQuery();
                    sqlConnection.Close();
                }
                catch (SqlException ex) { Console.WriteLine(ex.ToString()); }
