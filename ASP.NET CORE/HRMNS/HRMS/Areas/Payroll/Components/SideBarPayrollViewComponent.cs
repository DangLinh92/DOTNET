...
namespace HRMS.Areas.Payroll.Components
{
    public class SideBarPayrollViewComponent: ViewComponent
    {
        private IFunctionService _functionService;
        private IRoleAndPermisstionService _roleAndPermisstionService;
        private readonly IMemoryCache _memoryCache;

        public SideBarPayrollViewComponent(IFunctionService functionService, IRoleAndPermisstionService service, IMemoryCache memoryCache)
        {
        ...
        {
            string key = CacheKeys.MenuLst.ToString()+"-"+ ((ClaimsPrincipal)User).FindFirst(x => x.Type == "Roles").Value.Split(';')[0] + "-payroll";
        ...
        private RoleAndPermisstionViewModel GetPermistionByRole()
        {
            ...
            var functions = _functionService.GetAll("").Where(x => x.Area.ToLower().Contains("payroll")).ToList();
